...
<body>
    ...
    {{-- @section('content') --}}
    <div class="container">
        <h1>Daftar Competency Elements</h1>
        <a href="/manajement.createelement" class="btn btn-primary mb-3">Tambah Element</a>
        ...
        <table class="table">
            ...
            <tbody>
                @php
                $groupedElements = $elements->groupBy('standar.unit_title');
            @endphp

            @foreach ($groupedElements as $standard => $criteriaList)
                <tr>
                    <td rowspan="{{ $criteriaList->count() }}">{{ $loop->iteration }}</td>
                    <td rowspan="{{ $criteriaList->count() }}">{{ $standard }}</td>
                    <td>{{ $criteriaList[0]->criteria }}</td>
                    <td>
                        <!-- Tombol Edit -->
                        <a href="{{$criteriaList[0]->id }}" class="btn btn-sm btn-warning">
                            Edit
                        </a>
                        <!-- Tombol Hapus -->
                        <form action="{{  $criteriaList[0]->id }}" method="POST" style="display: inline-block;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Apakah Anda yakin ingin menghapus elemen ini?')">
                                Hapus
                            </button>
                        </form>
                    </td>
                </tr>
                @foreach ($criteriaList->skip(1) as $criteria)
                    <tr>
                        <td>{{ $criteria->criteria }}</td>
                        <td>
                            <!-- Tombol Edit -->
                            <a href="{{ route('manajement.editelement', $criteria->id) }}" class="btn btn-sm btn-warning">
                                Edit
                            </a>
                            <!-- Tombol Hapus -->
                            <form action="{{ route('manajement.deleteelement', $criteria->id) }}" method="POST" style="display: inline-block;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Apakah Anda yakin ingin menghapus elemen ini?')">
                                    Hapus
