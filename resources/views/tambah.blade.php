...
<head>
    ...
    <script>
        // Skrip untuk menampilkan atau menyembunyikan input tambahan berdasarkan role yang dipilih
        function toggleFields() {
            const role = document.getElementById('as').value;
            document.getElementById('admin_access_field').style.display = (role === 'admin') ? 'block' : 'none';
            document.getElementById('assessment_qualification_field').style.display = (role === 'assessor') ? 'block' : 'none';
...
<body>
    ...
    <div id="global-loader">
        ...
        <li class="submenu">
            <a href="javascript:void(0);"><img src="assets/img/icons/users1.svg" alt="img"><span>Setting</span> <span class="menu-arrow"></span></a>
            <ul>
    ...
    <div class="container mt-5">
        ...
        <form action="/tambah" method="POST">
            ...
            <div class="mb-3">
                <label for="as" class="form-label">Role</label>
                <select name="as" id="as" class="form-control" required onchange="toggleFields()">
                    <option value="">-- Pilih Role --</option>
                    <option value="admin" {{ old('as') == 'admin' ? 'selected' : '' }}>Admin</option>
                    <option value="assessor" {{ old('as') == 'assessor' ? 'selected' : '' }}>Assessor</option>
                    <option value="student" {{ old('as') == 'student' ? 'selected' : '' }}>Student</option>
                </select>
                @error('as')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
            <div class="mb-3" id="admin_access_field" style="display: none;">
                <label for="admin_access" class="form-label">Admin Access</label>
                <select name="admin_access" id="admin_access" class="form-control">
                    <option value="1" {{ old('admin_access') == '1' ? 'selected' : '' }}>Yes</option>
                    <option value="0" {{ old('admin_access') == '0' ? 'selected' : '' }}>No</option>
                </select>
            </div>
            <div class="mb-3" id="assessment_qualification_field" style="display: none;">
                <label for="assessment_qualification" class="form-label">Assessor Qualification</label>
                <select name="assessor_type" class="form-control">
                    <option value="internal" {{ old('assessor_type') == 'internal' ? 'selected' : '' }}>Internal</option>
                    <option value="external" {{ old('assessor_type') == 'external' ? 'selected' : '' }}>External</option>
                </select>
                <div class="mb-3">
            ...
            <div class="mb-3">
             ...
             <div id="student_fields" style="display: none;">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="grade_level" id="grade_level" class="form-control" value="{{ old('grade_level') }}">
                </div>
                  <div class="mb-3">
